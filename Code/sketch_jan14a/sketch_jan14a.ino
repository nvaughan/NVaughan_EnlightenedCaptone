...
unsigned long tmp = 0;
int count,randLight,chaseCount;
int roughBPM = 100;
...
void loop() 
{
  ...
  startMilli = millis();
  if(peakToPeak<55 && peakToPeak>35)
  {
  ...
  {
    int temp = startMilli - millis();
    beatIntervals[count] = temp;
    count +=1;
    if(count == 60)
      count = 1;
  }

  if(startMilli - millis() == BPMAccuracy10Seconds)
  {
    for(int i = 1; i<60;i++)
    {
      if(beatIntervals[i])
      {
       tmp = tmp+beatIntervals[i] /2;
      } 
      else
      {
        //tmp = tmp+beatIntervals[i] /2;
...
Serial.println(tmp);
//  while(startMilli-millis() <= 120000)
//  {
//    start(roughBPM);
//  }
}
